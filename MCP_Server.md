...
> 让 Claude 通过 MCP 协议直接控制 ANSYS Maxwell 电磁仿真软件。
...
| # | 位置 | 问题类型 | 问题描述 | 修复方案 |
|---|------|----------|----------|----------|
| 1 | 文件头注释 | **拼写错误** | `ANYS Maxwell` 应为 `ANSYS Maxwell` | 已修正 |
| 2 | `_safe()` / 各工具函数 | **重复的连接检查** | `_safe()` 内调用了 `_require_connected()`，而部分工具内部的 `_()` 也会通过 `get_design()` 间接检查，造成冗余；但 `new_project` 内部额外重复调用 `_require_connected()` 最为明显 | 将 `_require_connected()` 统一收到 `_safe()` 中，工具内不再重复 |
| 3 | `list_designs()` | **API 错误** | `proj.GetNumber()` 是获取项目数量的方法，不是设计数量；应使用 `proj.GetTopDesignList()` | 已修正为 `GetTopDesignList()` |
| 4 | `create_report()` | **参数传递错误** | `CreateReport()` 第2、3个参数分别是 `report_type` 和 Display Type，原代码硬传了 `"Data Table"` 作为 Display Type 与 `report_type` 混用，且最后一个数组参数格式有误 | 已修正参数结构 |
| 5 | `assign_vector_potential` 等多个边界/激励工具 | **`_args()` 滥用** | `Objects` 是列表类型，通过 `**{"Objects": [...]}` 传给 `_args()` 时被展开为 `"Objects:=", [...]`，这对于 ANSYS COM 接口是正确的；但 kwargs 展开与 `_args()` 内 `for k,v` 的处理逻辑一致，此处无 bug，代码可保留 | 无需修改，标记为误报 |
| 6 | `run_script()` | **未使用的参数** | `save_before: bool = True` 在函数体内完全没有被使用 | 已实现保存逻辑，或删除该参数 |
| 7 | `add_custom_material()` | **材料属性格式** | `AddMaterial()` 后用独立的 `EditMaterial()` 更新属性，格式 `(k):=` 为非标准写法，ANSYS COM 通常使用 `f"<key>:="` | 已修正属性键格式 |
| 8 | `get_status_resource()` 与 `get_maxwell_status()` | **代码重复** | 两个函数逻辑几乎完全相同 | 抽取公共逻辑为内部方法 |
| 9 | `draw_line()` | **参数默认值** | `start: list[str] = None` 类型提示不准确，应为 `Optional[list[str]] = None` | 已修正类型注解 |
| 10 | `pyproject.toml` | **依赖版本** | `mcp>=1.0.0` 过于宽松，建议加上上限 | 建议改为 `mcp>=1.0.0,<2.0.0` |
...
mcp = FastMCP(
    "ansys-maxwell",
    description="ANSYS Maxwell 电磁仿真控制 — 由 Claude 通过 MCP 驱动",
)
...
name = "mcp-maxwell"
version = "1.0.0"
description = "MCP Server for ANSYS Maxwell electromagnetic simulation"
...
```json
{
  "mcpServers": {
    "maxwell": {
      "command": "python",
      "args": ["C:/path/to/mcp-maxwell/server.py"],
      "env": {}
    }
  }
}
...
> **注意**：将 `C:/path/to/mcp-maxwell/server.py` 替换为实际的 `server.py` 绝对路径。推荐使用虚拟环境中的 Python 路径，如 `C:/path/to/venv/Scripts/python.exe`。

---

## 使用示例

配置好后，可以直接用自然语言让 Claude 操作 Maxwell。

### 示例 1 — 基本电磁铁仿真流程

```
> 帮我在 ANSYS Maxwell 中创建一个简单的电磁铁仿真

Claude 会依次调用：
1. connect_to_maxwell()
2. new_project()
3. create_design("Maxwell 3D", "Electromagnet", "Magnetostatic")
4. draw_cylinder(name="Core", material="steel_1010", ...)
5. draw_cylinder(name="Coil", material="copper", ...)
6. draw_box(name="WindingTurn1", material="copper", ...)
7. assign_current_excitation(objects=["WindingTurn1"], current="5A")
8. create_region(pad_x="50mm", ...)
9. assign_vector_potential(object_name="Region", value="0")
10. assign_force_parameter(objects=["Core"])
11. create_analysis_setup("Setup1", max_passes=15)
12. analyze("Setup1")
13. get_force("Force1")
```
...
```
> 帮我创建一个 8 极 48 槽永磁同步电机的 2D 仿真，计算转矩

Claude 会调用建模、材料分配、激励、求解、提取转矩等一系列工具
```

### 示例 3 — 参数化扫描

```
> 对气隙长度从 0.5mm 到 2mm 进行参数化扫描，步长 0.1mm

Claude 会设置变量、创建参数化扫描、执行并提取结果
```
...
| 设计要点 | 说明 |
|---------|------|
| **COM 接口** | 通过 `win32com` 连接 AEDT，需 Maxwell 已启动 |
| **参数格式** | `_args()` 辅助方法构建 ANSYS 风格的 `["NAME:xxx", "key:=", value]` 列表；直接传 ANSYS 列表风格的地方统一使用内联列表 |
| **错误处理** | 所有工具通过 `_safe()` 包装，统一捕获异常并返回友好错误信息 |
| **连接状态** | `connect_to_maxwell` 不经过 `_safe()` 预检，避免"未连接"时无法调用连接工具的死锁问题 |
| **状态复用** | `_get_status_dict()` 内部方法消除了 `get_maxwell_status` 与 `get_status_resource` 的重复逻辑 |
| **save_before** | `run_script()` 的 `save_before` 参数现已实际生效，执行前自动保存项目 |
| **工具分类** | 15 大类 60+ 工具，覆盖项目管理、建模、材料、边界、求解、结果全流程 |
| **脚本执行** | `run_script()` 和 `execute_console_command()` 允许执行任意 ANSYS 脚本作为兜底 |
